...
class MiniGameControllerTest {
    ...
    @MockBean
    private com.pokedex.pokedex_api.controller.mapper.MiniGameDtoMapper mapper;
    @MockBean
    ...
    void start_returns200() throws Exception {
        when(miniGameService.startRound(1L)).thenReturn(new MiniGameRound("pika.png"));
        mockMvc.perform(post("/v1/minigame/start")).andExpect(status().isOk());
    }

    @Test
    void answer_withBlankGuess_returns400() throws Exception {
        mockMvc.perform(post("/v1/minigame/answer")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new MiniGameAnswerRequest(""))))
